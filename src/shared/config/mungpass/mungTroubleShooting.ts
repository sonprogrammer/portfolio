...
export const mungTroubleshooting: TroubleshootingItem[] = [
  {
    ...
    reason:
      "매출 분석 결과를 조회할 때마다 새롭게 생성하는 구조였기 때문에 동일 날짜의 반복 조회도 모두 별도의 AI API 요청으로 처리됐습니다.",
    solution:
      "AI 매출 분석 결과를 DB에 저장하고 같은 날짜에는 저장된 결과를 재사용하도록 구성해 Gemini API가 하루에 한 번만 호출되도록 제한했습니다.",
    results: [
      "Chrome DevTools Network 탭 5회 측정 기준 AI 신규 분석 평균 10.16초 → DB 조회 평균 0.27초",
      "반복 조회 응답 시간 약 97% 단축",
      "동일 날짜의 중복 AI 분석 요청 방지",
    ],
  },
  {
    title: "재사용 컴포넌트의 캐시 잔상과 렌더링 타이밍 불일치",
    problem:
      "지도 API와 BottomSheet 컴포넌트를 여러 페이지에서 재사용할 때 이전 매장의 데이터가 잠시 남거나 새로운 데이터와 UI 렌더링 시점이 일치하지 않는 문제가 발생했습니다.",
    reason:
    ...
    results: [
      ...
      "매장 변경 시 내부 상태를 새로운 데이터 기준으로 초기화",
    ],
  },
  {
