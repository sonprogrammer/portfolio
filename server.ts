...
import { SendMessagePayload, SendMessageResult } from './src/entities/bnty/chat/model/chatSocketTypes';
...
import { startUpbitTickerStream, VC_TICKER_ROOM } from './server/vc/upbitTickerStream'
...
async function startServer() {
  ...
  const orderbookManager = new UpbitOrderbookManager(io)
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('vc:ticker:subscribe', () => {
      socket.join(VC_TICKER_ROOM)
      socket.emit('vc:ticker:snapshot', vcTickerStream.getSnapshot())
    })

    socket.on('vc:ticker:unsubscribe', () => {
    ...
    socket.on('mark-messages-read', async ({ chatRoomId, userId }: { chatRoomId: string, userId: string }) => {
      try {
        if (!Types.ObjectId.isValid(chatRoomId) || !Types.ObjectId.isValid(userId)) {
          return
        }
        ...
        const unreadMessagesIds = chatRoom.messages.filter(msg => msg.senderId.toString() !== userId &&
        ...
        await ChatRoomModel.updateOne(
          ...
          {
            $addToSet: {
              'messages.$[message].readBy': userObjectId
            }
          },
          {
            arrayFilters: [
        ...
        io.to(chatRoomId).emit('messages-read', {
          chatRoomId,
          userId,
          messageIds: unreadMessagesIds.map(msgId => msgId.toString())
        })
    ...
    socket.on('send-msg', async (payload: SendMessagePayload, callback: (res: SendMessageResult) => void) => {
      try {
        const { chatRoomId, senderId, senderRole, message } = payload
  ...
  httpServer.listen(port, hostname, () => {
    console.log(`서버 실행: http://${hostname}:${port}`);
  });
}
